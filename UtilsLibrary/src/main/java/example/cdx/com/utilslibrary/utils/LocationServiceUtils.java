    ...
    public static boolean isOpenLocService(final Context context) {
        ...
        if (context != null) {
            ...
            if (locationManager != null) {
                //1、判断GPS是否可用，通过GPS卫星定位，定位级别可以精确到街（通过24颗卫星定位，在室外和空旷的地方定位准确、速度快）
                isGps = locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER);
                //2、判断网络定位是否可用，通过WLAN或移动网络(3G/2G)确定的位置（也称作AGPS，辅助GPS定位。主要用于在室内或遮盖物（建筑群或茂密的深林等）密集的地方定位）
